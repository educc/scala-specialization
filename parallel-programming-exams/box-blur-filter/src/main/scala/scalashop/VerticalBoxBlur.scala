...
object VerticalBoxBlur extends VerticalBoxBlurInterface:
  ...
  def blur(src: Img, dst: Img, from: Int, end: Int, radius: Int): Unit = {
    val listVertical = (0 until src.height).toSeq
    def blurToCol(col: Int): Unit = {
      listVertical.foreach { y =>
        val pixel = boxBlurKernel(src, col, y, radius)
        dst.update(col, y, pixel)
      }
    }
    (from until end).foreach(blurToCol)
  }
