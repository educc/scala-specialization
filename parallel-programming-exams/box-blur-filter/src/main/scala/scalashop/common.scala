...
def clampImageHeight(y: Int)(implicit src: Img) : Int = clamp(y, 0, src.height-1)
def clampImageWidth(x: Int) (implicit src: Img) : Int = clamp(x, 0, src.width-1)
...
def boxBlurKernel(src: Img, x: Int, y: Int, radius: Int): RGBA = {
  implicit val srcimpl = src
  val leftX = clampImageWidth(x - radius)
  val leftY = clampImageHeight(y - radius)
